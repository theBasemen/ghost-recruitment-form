...
async function uploadFileToSupabase(file, fileName) {
    try {
        const response = await fetch(SUPABASE_URL + '/storage/v1/object/recruit_resumes/' + fileName, {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            ...
            return SUPABASE_URL + '/storage/v1/object/public/recruit_resumes/' + fileName;
        } else {
        ...
        form.addEventListener('submit', function(e) {
            ...
            async function submitApplication() {
                try {
                    ...
                    if (selectedFile) {
                        submitButton.innerHTML = '<span class="recruit_spinner"></span>Uploading resume...';
                        
                        // Create unique filename
                        var timestamp = Date.now();
                        var sanitizedName = fullName.replace(/[^a-zA-Z0-9]/g, '_');
                        var fileName = timestamp + '_' + sanitizedName + '_resume.pdf';
                        
                        console.log('Uploading file:', fileName);
                        resumeUrl = await uploadFileToSupabase(selectedFile, fileName);
                    ...
                    const response = await fetch(SUPABASE_URL + '/rest/v1/recruit_applications', {
                        method: 'POST',
                        headers: {
